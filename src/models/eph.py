...
class Network(nn.Module):
    ...
    def forward(self, obs, last_act, steps, hidden, relative_pos, comm_mask):
        ...
        obs = obs.view(seq_len * batch_size * num_agents, *self.obs_shape)  # 将obs展平
        last_act = last_act.view(seq_len * batch_size * num_agents, config.action_dim) # 将last_act展平 

        latent = self.obs_encoder(obs) # 将obs输入到obs_encoder中
        latent = torch.cat((latent, last_act), dim=1) # 将last_act拼接到latent中，这也就是为什么model里的latent_dim是hidden_dim+5
        latent = latent.view(seq_len, batch_size * num_agents, self.latent_dim) # 将latent从原本的形状转换为(seq_len, batch_size*num_agents, self.latent_dim)，以加入GRU

        hidden_buffer = []
        # 对seq_len中每步视野进行GRU + communicate
        for i in range(seq_len):
            ...
            # only hidden from agent 0，因为虽然FOV内有多个agent，但是当前训练只关注一个agent动作
        ...
        q_val = state_val + adv_val - adv_val.mean(1, keepdim=True) # dueling DQN
