...
class GlobalBuffer:
    ...
    def add(self, data: EpisodeData):
        ...
        """
        if data.actor_id >= 2:  # eps-greedy < 0.01
    ...
    def _sample_batch(self, batch_size: int) -> Tuple:
        ...
        with self.lock:
            ...
            # 需要逐一的是b_obs交换了通道和seqlen维度，为了便于模型并行训练
            data = (
...
class Learner:
    ...
    def _train(self):
        scaler = GradScaler()
        b_seq_len = torch.LongTensor(config.batch_size) # 创建了一个长度为config.batch_size的新张量，张量中的所有元素都初始化为0。
        b_seq_len[:] = config.burn_in_steps + 1 # 将张量中的所有元素都设置为config.burn_in_steps + 1
...
class Actor:
    def __init__(
        ...
        Args:
            ...
            epsilon: float, the epsilon value for epsilon-greedy
            learner: Learner, the learner object
            buffer: GlobalBuffer, the buffer object
        params:
            id: int, the id of the worker
            model: Network, the model object
            env: Environment, the environment object
            epsilon: float, the epsilon value for epsilon-greedy
            learner: Learner, the learner object
            global_buffer: GlobalBuffer, the buffer object
            max_episode_length: int, the max episode length for once simulation
            counter: int, the time of the actor simulation, used for updating the network         
        '''
        self.id = worker_id
        self.model = Network()
        self.model.eval()
    ...
    def run(self):
        ...
        obs, last_act, pos, local_buffer, goal = self._reset()
        ...
        while True:
            ...
            actions, q_val, hidden, relative_pos, comm_mask = self.model.step(
                ...
                torch.from_numpy(goal.astype(int)),
            )
    ...
    def _reset(self):
        self.model.reset()
        # obs: (num_agents, 6, 2 * self.obs_radius + 1, 2 * self.obs_radius + 1)
        # last_act: (num_agents, 5)
        # pos: (num_agents, 2)
        obs, last_act, pos = self.env.reset()
        goal = np.copy(self.env.goals_pos)
        local_buffer = LocalBuffer(
            self.id, self.env.num_agents, self.env.map_size[0], obs
        )
        return obs, last_act, pos, local_buffer, goal
